...
void __not_in_flash_func(update_output)(void)
{
  ...
  for (i = 0; i < playersCount; ++i)
  {
    ...
    if (players[0].button_mode != BUTTON_MODE_KB)
    {
      ...
      gc_report.stick_x    = furthest_from_center(gc_report.stick_x, players[i].output_analog_1x*LStick, 128);
      gc_report.stick_y    = furthest_from_center(gc_report.stick_y, players[i].output_analog_1y*LStick, 128);
